    ...
    <div className="space-y-6">
       {loading && posts.length === 0 ? (
        ...
        posts.map((post: any) => {
          ...
          const isCurrentUserPost = post.user?.id && currentUser?.id && post.user.id === currentUser.id;

        return (
          <div
            key={post.id}
            className={`max-w-2xl mx-auto p-4 rounded-lg shadow-lg mb-4 z-50 ${theme === "dark"
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center">
                <img
                  src={post.user?.userProfile?.avatar_path
